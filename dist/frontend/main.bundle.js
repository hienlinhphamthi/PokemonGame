...
}`, "",{"version":3,"sources":["webpack://./src/frontend/style.css"],"names":[],"mappings":"AAAA;IACI,sBAAsB;IACtB,kBAAkB;IAClB,+BAA+B;IAC/B,YAAY;AAChB;;AAEA;IACI,gCAAgC;AACpC","sourcesContent":[".gameItem{\r\n    border: 1px solid #ddd;\r\n    border-radius: 5px;\r\n    box-shadow: 5px 10px 10px wheat;\r\n    height: 80px;\r\n}\r\n\r\n.unmatched{\r\n    background-color: rgb(255, 0, 0);\r\n}"],"sourceRoot":""}]);
// Exports
...
class GameController {
    ...
    initGame(initData) {
        for (const item of initData) { // Khởi tạo trò chơi với dữ liệu ban đầu
            this.items.push(item); // Tạo các mục trong trò chơi dựa trên dữ liệu ban đầu
            this.items.push(new _models_GameItem__WEBPACK_IMPORTED_MODULE_2__.GameItem(item.id, item.divId, item.image));
        }
        let id = 1; //gán cho giá trị div.id và xác định status cho các phần tử(close)
    ...
    renderHTML(rootElement, item) {
        ...
        // </div>
        const divItem = document.createElement('div'); // khai báo 1 class name có giá trị col-2...
        divItem.className = 'col-2 gameItem m-2 p1 text-center';
        divItem.id = item.divId;
        divItem.addEventListener('click', this.processGameItemClicked); // gọi tới phương thức processgameitemclicked
        const imgItem = document.createElement('img'); // chèn thuộc tính img
        imgItem.src = `images/${item.image}`;
        imgItem.className = 'img-fluid invisible';
        item.imageElement = imgItem;
        divItem.appendChild(imgItem); // đưa div vào imgitem
    ...
    renderGameBoard() {
        this.shuffet();
        let boardDiv = this.element.querySelector('#board'); // gọi tới phương thức id board truyền kiểu htmlelement
        if (boardDiv) { // kiểm tra có tồn tại hay ko
            this.items.forEach(it => {
    ...
    isMatched(id, imgElement) {
        ...
        if (openedItems.length == 2) {
            ...
            if (checkMatchedFilter.length < 2) { // xử lí khi ko trùng khớp
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkasmtype"] = self["webpackChunkasmtype"] || [];
